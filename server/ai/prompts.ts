      ...
      properties: {
        ...
        face: {
          ...
          properties: {
            ...
            leftEyebrow: { type: "object", properties: { x: { type: "number" }, y: { type: "number" } }, required: ["x", "y"] },
            rightEyebrow: { type: "object", properties: { x: { type: "number" }, y: { type: "number" } }, required: ["x", "y"] },
...
export function characterAnalysisPrompt(width: number, height: number): string {
  return [
    ...
    jointHints(),
    "partRegions: give a tight closed polygon for every visible semantic part. Include head, torso, each arm, hand, finger/thumb group, leg, foot, eyebrow, hair, and accessory when present. Preserve left/right names.",
    "face: left/right eye, left/right eyebrow, and mouth anchor points when visible; otherwise omit only the missing features.",
