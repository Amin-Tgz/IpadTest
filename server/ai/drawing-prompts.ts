...
export function drawingAnalysisPrompt(context: DrawingPromptContext): string {
  return [
    ...
    "When the action is move, climb, or jump, the reaction is spoken AFTER the hero arrives or lands, so phrase it as having arrived (e.g. «هوف! رسیدم بالا؛ از این بالا همه‌جا پیداست!» or «هوپ! پریدم پایین؛ دیدی چه فرودی بود؟»), not as a plan.",
