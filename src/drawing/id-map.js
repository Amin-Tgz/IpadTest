...
export function encodeIdColor(n) {
    const r = (n >> 16) & 0xff;
    const g = (n >> 8) & 0xff;
    const b = n & 0xff;
    return `#${[r, g, b].map((v) => v.toString(16).padStart(2, "0")).join("")}`;
}
export function decodeIdColor(hex) {
...
export class IdMap {
    ...
    getCanvas(region) {
        ...
        if (!ctx)
            throw new Error("id-map canvas unavailable");
        return ctx;
    }
    sampleStrokesInRegion(store, regionWorld, minPixels = 6) {
    ...
    sampleStrokesInPolygon(store, polygonWorld, minPixels = 4) {
        if (polygonWorld.length < 3)
            return new Set();
    ...
    countColors(ctx, region, minPixels) {
...
export function strokesInsideBox(strokes, box) {
    const result = new Set();
    for (const stroke of strokes) {
        const inside = stroke.points.filter((p) => p.x >= box.x && p.x <= box.x + box.width && p.y >= box.y && p.y <= box.y + box.height).length;
