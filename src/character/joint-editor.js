...
export function hitTestBox(p, box, radius = EDGE_RADIUS) {
    const onEdgeX = p.x >= box.x - radius && p.x <= box.x + box.width + radius;
    const onEdgeY = p.y >= box.y - radius && p.y <= box.y + box.height + radius;
    if (onEdgeY && Math.abs(p.x - box.x) <= radius)
        return { mode: "resize_w", jointIndex: -1 };
    if (onEdgeY && Math.abs(p.x - (box.x + box.width)) <= radius)
        return { mode: "resize_e", jointIndex: -1 };
    if (onEdgeX && Math.abs(p.y - box.y) <= radius)
        return { mode: "resize_n", jointIndex: -1 };
    if (onEdgeX && Math.abs(p.y - (box.y + box.height)) <= radius)
        return { mode: "resize_s", jointIndex: -1 };
    const inside = p.x >= box.x - radius &&
        p.x <= box.x + box.width + radius &&
        p.y >= box.y - radius &&
        p.y <= box.y + box.height + radius;
    if (inside)
        return { mode: "move", jointIndex: -1 };
    return { mode: "toggle", jointIndex: -1 };
}
export function hitTestJoint(p, joints, radius = HIT_RADIUS) {
    let best = -1;
    let bestDist = radius;
    ...
    if (best >= 0)
        return { mode: "joint", jointIndex: best };
    return { mode: "toggle", jointIndex: -1 };
}
export class JointEditor {
    ...
    excluded = new Set();
    drag = null;
    onStageChange = () => void 0;
    constructor(store, getViewport) {
        this.store = store;
        this.getViewport = getViewport;
    }
    begin(box, manifest, filter = isUserStroke) {
        this.box = { ...box };
        this.manifest = structuredClone(manifest);
        this.filter = filter;
        this.excluded = new Set(this.store
            .all()
    ...
    applyInclusionFromBox() {
        if (!this.manifest || !this.box)
            return;
        const strokes = this.store.all().filter((s) => s.active && this.filter(s));
        const included = new Set();
        for (const stroke of strokes) {
            const inside = stroke.points.filter((p) => p.x >= this.box.x && p.x <= this.box.x + this.box.width && p.y >= this.box.y && p.y <= this.box.y + this.box.height).length;
    ...
    nearestStroke(p) {
        ...
        for (const stroke of this.store.all()) {
            if (!stroke.active || !this.filter(stroke))
                continue;
    ...
    finalize() {
        ...
        this.manifest.parts = this.manifest.parts.filter((part) => part.strokeIds.some((id) => !this.excluded.has(id)));
    }
    manifestReady() {
        if (!this.manifest)
            return null;
        if (!verifyManifest(this.manifest))
            return null;
        return this.manifest;
    }
    draw(ctx, camera) {
        if (!this.box || !this.manifest)
            return;
        ...
        if (this.stage === "joints") {
            for (const joint of this.manifest.joints) {
                const color = joint.confidence >= 0.85 ? PALETTE.primaryInk : joint.confidence >= 0.6 ? PALETTE.warning : PALETTE.error;
                ctx.fillStyle = color;
                ctx.beginPath();
