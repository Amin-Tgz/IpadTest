...
## iPad Silent-mode audio checkpoint

- **Symptom:** no voice on iPad although the speech log showed `contextState: 'running'`, `playback_started`, and `playback_ended` after the full clip with a live analyser level. Nothing in the audio path had changed; the sound was reaching Web Audio but not the speaker.
- **Cause:** iPadOS routes Web Audio through an "ambient" audio session that Silent mode mutes, while HTML media uses a "playback" session that ignores it. A muted context still reports `running` and every clip still "plays".
- **Fix:** `src/audio/audio-session.ts` asks for the playback session inside the Start tap, before any await: `navigator.audioSession.type = "playback"` (Safari 16.4+), or on older Safari a silent looping `<audio>` element (a generated 0.25s silent WAV data URI) that holds the session and pauses while the page is hidden. Speech and sound effects share the same context, so both benefit.
- Every speech diagnostic now carries `audioSession` (`playback`, `silent-element`, or `unsupported`), and `speech_unlocked` records which request was made, so an iPad `?debug=speech` log shows at a glance whether the session was granted.
- Verification: strict typecheck passed, 47 test files / 276 tests passed (session request, single silent element, pause/resume on visibility, fallback when the session refuses). In Chromium, which has no Audio Session API, the fallback engaged (`audioSession: silent-element`) and speech played with no errors. Windows WebKit has no Audio Session API or usable Web Audio, so final confirmation needs the iPad itself.

## Voice timing, fishing rod grip, jump down, and ponytail checkpoint

- **Some lines showed only the bubble.** Timed against the live provider, a clean Gemini take took 5.5–7.5s to generate while a line gave up after 8s, and the server may need a second take (the speech guard), so roughly any slow line was silent. A line now waits up to 18s for its voice (`LINE_WAIT_MS`). The generation request is shared and outlives the line (40s): a cancelled or timed-out line no longer aborts it, a line that joins an earlier prime keeps its own budget, and late audio still lands in the cache. Free-play reactions are primed as soon as the analysis returns, so a move/climb/ride reaction is ready on arrival; the shoe-complete and fish-ending lines are preloaded. Every tap also resumes a context iPadOS suspended or interrupted (`keepAlive`).
- **The rod stuck to the head / floated where it was drawn.** The tutorial tool trusted the provider's joint, anchor, and category. Reproduced with Playwright: `attachTo: head`, or `right_hand` with the anchor on the hand joint while the ink was above the head, left the rod hovering over the head. `equipFishingTool` now always uses the right hand, grips the ink point nearest the hand, and turns the rod about that grip to point up toward the pond (shortened to 1.4× hero height if needed). In free play a `held_tool` always goes in a hand, and an anchor that is not on the ink snaps to the nearest ink point.
- **The tool stayed after «حالا نوبت دنیای توست».** Put-away only removed `held_tool` attachments; a rod the provider called `other` became a "wearable". Anything attached to a hand is now put away, and hand tools are also cleared when a session is restored into the tool request or after the ending.
- **No jump down from drawn stairs.** A down arrow's affordances («step down from the stairs») matched the stairs pattern, so the arrow became a climbable staircase: a down move/climb climbed onto the arrow and a jump landed on it. Arrows, motion lines, and writing now never get a physics shape; a downward move or climb toward one becomes a jump. A down jump lands past the drop edge, or at the arrow when it points farther (`dropLanding`), and a leap only arrives once it has landed (`landFirst`) — arriving mid-air used to drop the hero back onto the ledge.
- **Ponytail.** The hero has an authored ponytail and hair tie (part `hair`, bound to the head). `StrandSway` gives it spring-damped secondary motion: it trails behind movement and settles when the hero stops.
- Verification: strict typecheck passed, 48 test files / 294 tests passed (line wait past one take, joining a primed generation, late audio cached, cancel not aborting a shared request; arrow/instruction shapes and downward intent; drop landing; grip snapping, aiming, hand-held put-away; ponytail binding and sway), production build passed. Playwright (Chromium 1024×768) against the running app, baseline first and then after the fix, screenshots and page logs checked: with mocked analyses of the rod as `other` on the hand joint and as `held_tool` on the head, the rod was held in the right hand, cast, and removed after the ending (`held_tools_put_away`), where before it hovered above the head and stayed; the live model returned `fishing_rod` on `right_hand` and the same held/removed result, and its reaction needed 7.2s of generation yet still played (7/7 lines played in every run). On top of drawn stairs, a down arrow answered with `move`, `climb`, or `jump` (mocked) and with `jump` (live model) now lands once on the ground past the edge (`character_landed` y≈502) and then speaks; before, the arrow became stairs and the hero climbed or landed onto it. Move/climb reactions were primed and started within 2ms of arrival.
